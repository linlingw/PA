...
void difftest_step(uint32_t eip) {
  ...
  if(r.eax!=cpu.eax){
    diff=true;
    printf("eax diff! nemu.eip = 0x%x,\t nemu.eax = 0x%x,\t qemu.eax = 0x%x\n", cpu.eip, cpu.eax, r.eax);
  }
  if(r.ecx!=cpu.ecx){
    diff=true;
    printf("ecx diff! nemu.eip = 0x%x,\t nemu.ecx = 0x%x,\t qemu.ecx = 0x%x\n", cpu.eip, cpu.ecx, r.ecx);
  }
  if(r.edx!=cpu.edx){
    diff=true;
    printf("edx diff! nemu.eip = 0x%x,\t nemu.edx = 0x%x,\t qemu.edx = 0x%x\n", cpu.eip, cpu.edx, r.edx);
  }
  if(r.ebx!=cpu.ebx){
    diff=true;
    printf("ebx diff! nemu.eip = 0x%x,\t nemu.ebx = 0x%x,\t qemu.ebx = 0x%x\n", cpu.eip, cpu.ebx, r.ebx);
  }
  if(r.esp!=cpu.esp){
    diff=true;
    printf("esp diff! nemu.eip = 0x%x,\t nemu.esp = 0x%x,\t qemu.esp = 0x%x\n", cpu.eip, cpu.esp, r.esp);
  }
  if(r.ebp!=cpu.ebp){
    diff=true;
    printf("ebp diff! nemu.eip = 0x%x,\t nemu.ebp = 0x%x,\t qemu.ebp = 0x%x\n", cpu.eip, cpu.ebp, r.ebp);
  }
  if(r.esi!=cpu.esi){
    diff=true;
    printf("esi diff! nemu.eip = 0x%x,\t nemu.esi = 0x%x,\t qemu.esi = 0x%x\n", cpu.eip, cpu.esi, r.esi);
  }
  if(r.edi!=cpu.edi){
    diff=true;
    printf("edi diff! nemu.eip = 0x%x,\t nemu.edi = 0x%x,\t qemu.edi = 0x%x\n", cpu.eip, cpu.edi, r.edi);
  }
  if(r.eip!=cpu.eip){
    diff=true;
    printf("eip diff! nemu.eip = 0x%x,\t nemu.eip = 0x%x,\t qemu.eip = 0x%x\n", cpu.eip, cpu.eip, r.eip);
  }
